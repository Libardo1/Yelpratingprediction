learningCurve <- function(usersKeys, businessKeys, y, theta1, theta2, alpha, users.env, business.env, checkin.env, trainIndices, valIndices, lambda){
  ...
  for (i in 1:m){
    ...
    model <- stochasticNeuralNetEnvironments(usersKeys[indicesVector], businessKeys[indicesVector], 
                                             y[indicesVector], theta1, theta2, alpha, users.env, business.env, checkin.env, lambda)    
