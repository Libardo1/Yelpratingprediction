validationCurveEnvironments <- function(usersKeys, businessKeys, y, theta1, theta2, alpha, users.env, business.env, checkin.env, trainIndices, valIndices){
  ...
  for (i in 1:length(lambdaVec)){
    lambda <- lambdaVec[i]
    model <- stochasticNeuralNetEnvironments(usersKeys[trainIndices], businessKeys[trainIndices], 
                                      y[trainIndices], theta1, theta2, alpha, users.env, business.env, checkin.env, lambda)
